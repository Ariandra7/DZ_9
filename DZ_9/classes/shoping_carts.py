...
class ShoppingCart:
    ...
    """
    def __init__(self, customer_name, user_role):
        self.items = []
    ...
    def get_details(self):
        ...
        Также добавляет информацию о покупателе и роли пользователя.
        """
        details = f"Покупатель {self.customer_name} приобрел:\n"
        for item in self.items:
            details += f"{item['Продукт'].get_details()}, Количество: {item['количество']}\n"
        details += f"Общее: {self.get_total()} руб. "
        details += f"Зарегистрирован покупки пользователь {self.user_role}"
